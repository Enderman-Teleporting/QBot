...
class MemberChange {
    ...
    @Listener
    suspend fun FriendDecreaseEvent.tellDecrease(){
        Log_settler.writelog("Your bot's friend ${friend().username}(${friend().id}) left it")}

    @Listener
    suspend fun FriendIncreaseEvent.tellIncrease(){
        Log_settler.writelog("Your bot's new friend: ${friend().username}(${friend().id})")}
}